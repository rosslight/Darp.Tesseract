...
{
    internal MatrixXD(MatrixStorage storage, MatrixLayout layout, int offset = 0) : base(storage, layout, offset) { }

    private MatrixXD(Memory<double> memory, MatrixLayout layout, MemoryManager<double>? owner = null) : base(memory, layout, owner) { }
    public MatrixXD(int rows, int columns) : base(rows, columns) { }
    /// <summary>Maps caller-owned memory without copying or taking responsibility for its disposal.</summary>
    public static MatrixXD CreateFromMemory(Memory<double> memory, int rows, int columns, int? columnStride = null, int rowStride = 1)
    {
        var layout = MatrixLayout.Create(rows, columns, rowStride, columnStride ?? Math.Max(1, checked(rows * rowStride)));
        ...
        int rowStride = 1)
    {
        ...
        try
        {
            layout = MatrixLayout.Create(rows, columns, rowStride, columnStride ?? Math.Max(1, checked(rows * rowStride)));
        }
        catch
        {
    ...
    public MatrixXD Block(int row, int column, int rows, int columns)
    {
        ...
        return new MatrixXD(Storage, layout, layout.Extent == 0 ? Offset : checked(Offset + Layout.Offset(row, column)));
    }
    public MatrixXD Row(int row) => Block(row, 0, 1, Columns);
    public VectorXD Column(int column) => new(Storage, Layout.Block(0, column, Rows, 1), Rows == 0 ? Offset : checked(Offset + column * ColumnStride));
