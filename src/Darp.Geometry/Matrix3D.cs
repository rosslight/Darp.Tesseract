...
public sealed class Matrix3D : GeometryObject, IMatrixD, IReadOnlyMatrix3D
{
    ...
    IReadOnlyMatrixD IReadOnlyMatrix3D.Row(int row) =>
        new ReadOnlyMatrix(Storage, Layout.Block(row, 0, 1, Columns), checked(Offset + row * RowStride));

    public Vector3D Column(int column) =>
        new(Storage, Layout.Block(0, column, 3, 1), checked(Offset + column * ColumnStride));

    IReadOnlyVector3D IReadOnlyMatrix3D.Column(int column) =>
