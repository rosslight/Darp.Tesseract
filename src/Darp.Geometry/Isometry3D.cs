...
{
    internal Isometry3D(MatrixStorage storage, MatrixLayout layout, int offset = 0) : base(storage, layout, offset) { }

    private Isometry3D(Memory<double> memory, MatrixLayout layout) : base(memory, layout) { }
    public Isometry3D() : base(4, 4)
    {
        for (int i = 0; i < 4; i++) SetValue(i, i, 1);
    }
    public Isometry3D(IReadOnlyQuaternionD rotation, IReadOnlyVector3D translation) : this()
    {
        try { SetRotation(rotation); SetTranslation(translation); }
        catch { Dispose(); throw; }
    }
    internal static Isometry3D FromOwnedMatrix(MatrixXD matrix) { using (matrix) return new(matrix.Storage, matrix.Layout.Require(4, 4), matrix.Offset); }
    ...
    {
        get { using var matrix = RotationMatrix; return matrix.ToQuaternion(); }
        set => SetRotation(value);
    }
    public void SetTranslation(IReadOnlyVector3D value)
    {
        double x = value.X, y = value.Y, z = value.Z;
        SetValue(0, 3, x); SetValue(1, 3, y); SetValue(2, 3, z);
    }
    ...
    public void SetRotationMatrix(IReadOnlyMatrix3D value)
    {
        ...
        for (int c = 0; c < 3; c++)
            for (int r = 0; r < 3; r++) SetValue(r, c, copy[c * 3 + r]);
    }
    public static Isometry3D operator *(Isometry3D left, IReadOnlyIsometry3D right) => GeometryExtensions.Multiply(left, right);
