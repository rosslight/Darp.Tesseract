...
{
    internal ReadOnlyIsometry3D(MatrixStorage storage, MatrixLayout layout, int offset = 0) : base(storage, layout, offset) { }

    public IReadOnlyVector3D Translation => new ReadOnlyVector3D(Storage, Layout.Block(0, 3, 3, 1), checked(Offset + 3 * ColumnStride));
